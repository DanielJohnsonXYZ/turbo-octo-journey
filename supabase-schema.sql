-- Relationship Management Platform Schema
...
CREATE TABLE research_items (
  ...
  source TEXT CHECK (source IN ('linkedin', 'news', 'twitter', 'web', 'manual')),
  source_name TEXT,
  title TEXT NOT NULL,
  summary TEXT,
  url TEXT,
...
CREATE TABLE interactions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  contact_id UUID REFERENCES contacts(id) ON DELETE CASCADE,
  type TEXT CHECK (type IN ('outreach', 'response', 'meeting', 'note', 'call')),
  channel TEXT CHECK (channel IN ('email', 'linkedin', 'whatsapp', 'phone', 'in_person', 'other')),
  content TEXT,
  outcome TEXT,
...
-- RLS Policies (if using Supabase Auth later)
